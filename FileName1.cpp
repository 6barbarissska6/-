...
void cpp1main()
{
    ...
    while (true) {
        ...
        cout << "1) Вывести, сколько памяти (в байтах) на вашем компьютере отводится под различные типы данных со спецификаторами и без." << endl;
        ...
        cout << "5) идз 13 для второго задания" << endl;
        cout << "6) идз 13 для третьего задания" << endl;
        cout << "7) идз 13 для четвертого задания" << endl;
        cout << "идз 13:если число отрицательное, то необходимо поменять значения четных битов на 0, если положительное, то поменять значения нечетных битов на 0. " << endl;
        int choose, num = -1;
        cin >> choose;
        ...
        case 2: {
            ...
            cin >> num;
            unsigned mask = 1 << ((sizeof(int) * 8) - 1); // Создаем маску для проверки каждого бита
            ...
            for (int i = 0; i < sizeof(int) * 8; i++) {
                ...
                if (i % (sizeof(int) * 8) - 1 == 0) // Отделение знаковой части
                {
                    cout << " ";
                }
